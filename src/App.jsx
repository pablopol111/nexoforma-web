...
function loadState() {
  try {
    ...
    if (!raw) return { clients: {}, lastToken: "" };
...
function Logo({ className = "w-10 h-10" }) {
  return (
    <svg viewBox="0 0 64 64" className={className} aria-hidden="true">
      <defs>
        <linearGradient id="nxfGrad" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#38bdf8" />
          <stop offset="100%" stopColor="#22c55e" />
        </linearGradient>
      </defs>
      <rect x="6" y="6" width="52" height="52" rx="14" fill="#0f172a" stroke="#334155" strokeWidth="2" />
      <path d="M18 42V21h7l8 12 8-12h7v21h-6V31l-9 13-9-13v11h-6Z" fill="url(#nxfGrad)" />
    </svg>
  );
}
...
function App() {
  ...
  const currentClient = state.clients[activeToken] || null;
  const profiles = currentClient?.profiles || [];
  ...
  function patchProfile(profileId, patch) {
    updateState((prev) => {
      const client = prev.clients[activeToken];
      ...
      return {
        ...
        clients: {
          ...
          [activeToken]: {
            ...client,
            profiles: client.profiles.map((p) => (p.id === profileId ? { ...p, ...patch } : p)),
          },
        },
      };
    });
  ...
  function addProfile() {
    if (!currentClient || currentClient.profiles.length >= MAX_PROFILES) return;
    const newProfile = defaultProfile(currentClient.profiles.length + 1);
    updateState((prev) => ({
      ...prev,
      clients: {
        ...prev.clients,
        [activeToken]: {
          ...
          profiles: [...prev.clients[activeToken].profiles, newProfile],
        },
      },
    }));
  ...
  function deleteProfile(profileId) {
    if (!currentClient || currentClient.profiles.length <= 1) return;
    updateState((prev) => ({
      ...prev,
      clients: {
        ...prev.clients,
        [activeToken]: {
          ...prev.clients[activeToken],
          profiles: prev.clients[activeToken].profiles.filter((p) => p.id !== profileId),
        },
      },
    }));
  }
  ...
  function saveEntry() {
    ...
    const row = {
      ...
      steps,
      comment: entryForm.comment?.trim() || "",
    };
  ...
  function removeEntry(date) {
    updateState((prev) => ({
      ...
      clients: {
        ...
        [activeToken]: {
          ...prev.clients[activeToken],
          profiles: prev.clients[activeToken].profiles.map((p) =>
  ...
  function exportWord() {
    ...
    const sections = selectedProfiles
      .map((profile) => {
    ...
    a.href = url;
    a.download = `NexoForma_${currentClient.clientName.replace(/\s+/g, "_")}.doc`;
        ...
        <div className="mx-auto max-w-[1700px] px-4 md:px-6 py-4 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-2xl bg-slate-800 px-4 py-2 text-sm font-semibold text-slate-100 hover:bg-slate-700 disabled:opacity-50"
            >
              <UserPlus className="w-4 h-4" /> Añadir perfil
            </button>
            <button
              onClick={exportWord}
        ...
        {currentProfile && (
          <>
            <div className="grid xl:grid-cols-2 gap-6">
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="rounded-2xl bg-slate-800 px-4 py-2 text-sm font-semibold hover:bg-slate-700"
                    >
                      {profileEdit ? "Bloquear" : "Editar perfil"}
                    </button>
                    {profiles.length > 1 && (
                      <button
                        onClick={() => deleteProfile(currentProfile.id)}
                        className="rounded-2xl bg-red-500/20 px-4 py-2 text-sm font-semibold text-red-300 hover:bg-red-500/30"
                      >
                        Eliminar perfil
                      </button>
                    )}
                ...
                <div className="mt-6 grid md:grid-cols-2 gap-4">
                  <Field label="Nombre del perfil">
                    <input
                      ...
                      onChange={(e) => patchProfile(currentProfile.id, { name: e.target.value })}
                      className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 disabled:text-slate-400 outline-none focus:border-sky-400"
                    />
                  </Field>
                  <Field label="Peso de referencia (kg)">
                    <input
                      value={currentProfile.referenceWeight || ""}
                      disabled={!profileEdit}
                      onChange={(e) => patchProfile(currentProfile.id, { referenceWeight: parseNum(e.target.value) })}
                      className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 disabled:text-slate-400 outline-none focus:border-sky-400"
                    />
                  </Field>
                  <Field label="Peso objetivo (kg)">
                    <input
                      value={currentProfile.targetWeight || ""}
                      disabled={!profileEdit}
                      onChange={(e) => patchProfile(currentProfile.id, { targetWeight: parseNum(e.target.value) })}
                      className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 disabled:text-slate-400 outline-none focus:border-sky-400"
                    />
                  </Field>
                  <Field label="Altura (cm)">
                    <input
                      value={currentProfile.heightCm || ""}
                      disabled={!profileEdit}
                      onChange={(e) => patchProfile(currentProfile.id, { heightCm: parseNum(e.target.value) })}
                      className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 disabled:text-slate-400 outline-none focus:border-sky-400"
                    />
                  </Field>
                </div>
              ...
              <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-lg">
                ...
                <div className="mt-6 grid md:grid-cols-2 gap-4">
                  ...
                  <Field label="Número de pasos">
                    <input
                      value={entryForm.steps}
                      onChange={(e) => setEntryForm((f) => ({ ...f, steps: e.target.value }))}
                      className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 outline-none focus:border-sky-400"
                    />
                  </Field>
                ...
                <div className="mt-5 flex flex-wrap gap-3">
                  <button onClick={saveEntry} className="rounded-2xl bg-sky-500 px-5 py-3 font-semibold text-white hover:bg-sky-400">
            ...
            <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-4">
              ...
              <StatCard
                title="Movimiento"
                value={prevDelta === null ? "Sin base" : prevDelta < 0 ? "Descenso" : prevDelta > 0 ? "Incremento" : "Estable"}
                detail={prevDelta === null ? "Aún no hay comparativa" : prevDelta < 0 ? "La última lectura refleja bajada" : prevDelta > 0 ? "La última lectura refleja subida" : "Sin cambios"}
                colorClass={prevDelta === null ? "text-slate-100" : prevDelta < 0 ? "text-emerald-400" : prevDelta > 0 ? "text-red-400" : "text-slate-100"}
              />
            </div>
              ...
              <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-lg">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <select value={chartGroupBy} onChange={(e) => setChartGroupBy(e.target.value)} className="rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3">
                      <option>Día</option>
                      <option>Semana</option>
                      <option>Mes</option>
                      <option>Año</option>
                    </select>
                    <select value={exportScope} onChange={(e) => setExportScope(e.target.value)} className="rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3">
                      <option>Perfil actual</option>
                  ...
                  {chartData.length ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={chartData} margin={{ top: 10, right: 25, left: 0, bottom: 10 }}>
                        ...
                        {chartMetric !== "Peso" && (
                          <Line yAxisId={chartMetric === "Ambos" ? "right" : "left"} type="monotone" dataKey="steps" name="Pasos" stroke="#22c55e" strokeWidth={3} strokeDasharray="8 6" dot={{ r: 4 }} />
                        )}
                      </LineChart>
                    </ResponsiveContainer>
                  ) : (
                  ...
                  <table className="min-w-full text-sm">
                    <thead className="bg-slate-950 text-slate-300">
                      <tr>
                        ...
                        <th className="px-3 py-3 text-left">Pasos</th>
                        <th className="px-3 py-3 text-left">Comentario</th>
